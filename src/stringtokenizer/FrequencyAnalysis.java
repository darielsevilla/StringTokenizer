...
public class FrequencyAnalysis {
    static String  singlesOutput = "./datafiles/individuales.txt",
                paresOutput = "./datafiles/pares.txt",
                linea = "";
    static int i = 0, minimum= 5000, top = 20;
        ...
        try (BufferedReader lectura = new BufferedReader(new FileReader(paresOutput))) {
            while ((linea = lectura.readLine()) != null) {
                ...
                if (!linea.contains("$")) {
                    ...
                    if (Integer.parseInt(parts[1]) >= minimum)
                        pares.add(new wordFrequency(contenido.replace(",", " "), Integer.parseInt(parts[1])));
                }
